...
# Check a TREC 2015 clinical decision support track submission for various
# common errors:
#      * extra fields
#      * multiple run tags
...
# If you have a master list of collection document ids, this
# script will read it and check that all your ids are valid.
...
# document ids.  The master list of doc ids is posted on the tracks page
# in the active participants' section of the TREC web site.
# You are strongly encouraged to make use of this check.

# Change these variable values to the full path of the list of document ids
# and the directory where the error log should be put
my $docno_loc = "./cds-docnos.txt";
my $errlog_dir = ".";
...
while ($line = <RESULTS>) {
	...
	&error("Doc id `$docno' retrieved more than once for topic $topic");
	next;
